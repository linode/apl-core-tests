...
export function computeTag(branchTags: string[], releaseSeries: ReleaseSeries, promote: boolean): string {
  return promote
    ? computeStableTag(branchTags, releaseSeries)
    : computeNextRcTag(branchTags, releaseSeries)
}
...
function main() {
  ...
  const tag = computeTag(branchTags, releaseSeries, promote)
